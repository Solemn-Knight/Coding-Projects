...
#sounds
secure_piece_sound = pygame.mixer.Sound(os.path.join(data_dir, "moderntechselect.wav"))
lineclear_sound = pygame.mixer.Sound(os.path.join(data_dir, "retrogamenotification.wav"))
rotate_sound = pygame.mixer.Sound(os.path.join(data_dir, "metalbuttonpush.wav"))
move_sound = pygame.mixer.Sound(os.path.join(data_dir, "gameballtap.wav"))
move_sound.set_volume(0.02)
...
def attempt_clear_rows(starty):
    ...
    for iy in range(BOARD_HEIGHT-starty):
        ...
        if filledrow:
            ...
            play_lineclearsound = True
            break
    if play_lineclearsound:
        pygame.mixer.Sound.play(lineclear_sound)
...
def display_developerinfo():
    ...
    FPS_text = font.render(str(elapsed_time_delta) + "ms / Frame", True, (255,255,255))
...
while running:
    ...
    if game_state == "menu":
        ...
        if board_state == "active":
            ...
            if cw_click:
                cw_click = False
                success = attempt_rotate_piece(1)
                if success:
            ...
            if frame_index - piece_move_timestamp > 30:
                secure_piece()
                pygame.mixer.Sound.play(secure_piece_sound)
                piece_move_timestamp = frame_index
                active_piece = False
                piece_secured = True
    ...
    pygame.display.flip()
    elapsed_time_delta = clock.tick(600) #600 fps
