...
NEXTQUEUE_ORIGIN_X = 800
NEXTQUEUE_ORIGIN_Y = 50
def render_nextqueue():
    displayed_pieces = 0
    bag_index = 0
    marble_index = 0
    while displayed_pieces < 5:
    ...
    while not fail_rotation:
        ...
        for i in range(4):
            ...
            if testcase != 0:
                """
                print(rotation_type)
                print(reposition_type)
                print (PIECE_ROTATION_REPOSITIONS[reposition_type])
                print (PIECE_ROTATION_REPOSITIONS[reposition_type][rotation_type])
                ...
                #if get_row(new_position+x) == get_row(new_position):
                new_position = new_position+x
                #elif get_row(new_position+x+1) == get_row(new_position):
                #    new_position = new_position+x+1
            ...
            #See if new unit is at a different height level in relation to the active position than it should be
            #print(unit_displacement)
            #print(abs(get_row(unit_index) - get_row(new_position)))

            desired_rowdifference = math.floor((abs(unit_displacement)+3)/10)
            """
            if unit_displacement > 0:
                desired_rowdifference-=1
            if unit_displacement % 10 == 0:
                desired_rowdifference -=1
            """
            #print(desired_rowdifference)
...
def display_developerinfo():
    global active_position
    global font
    active_position_text = font.render(str((elapsed_time - gravity_timestamp)/1000), True, (255,255,255))
...
while running:
    ...
    if game_state == "menu":
        ...
        if board_state == "active":
            ...
            if frame_index - piece_move_timestamp > 60:
                secure_piece()
                piece_move_timestamp = frame_index
                active_piece = False
    ...
    pygame.display.flip()
    elapsed_time += clock.tick(600)  # limits FPS to 60
    frame_index +=0.1
